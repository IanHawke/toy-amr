...
def rea_method(reconstruction, flux_solver):
    def rea_solver(tl, simulation):
        cons_m, cons_p = reconstruction(tl.cons, simulation)
        flux = flux_solver(cons_m, cons_p, simulation, tl)
        rhs = numpy.zeros_like(flux)
        rhs[:,1:-1] = 1/tl.grid.dx * (flux[:,1:-1] - flux[:,2:])
        return rhs
    return rea_solver
    
def rea_method_prim(reconstruction, flux_solver):
    def rea_solver(tl, simulation):
        prim_m, prim_p = reconstruction(tl.prim, simulation)
...
def rea_method_source(reconstruction, flux_solver, source_term):
    def rea_solver(tl, simulation):
        cons_m, cons_p = reconstruction(tl.cons, simulation)
        flux = flux_solver(cons_m, cons_p, simulation, tl)
        rhs = source_term(tl.cons, tl.prim, tl.aux)
        rhs[:,1:-1] += 1/tl.grid.dx * (flux[:,1:-1] - flux[:,2:])
        return rhs
    return rea_solver
    
def fvs_method(order, slow_source=None):
    def fvs_solver(tl, simulation):
        alpha = simulation.model.max_lambda(tl.cons, tl.prim, tl.aux)
        flux = simulation.model.flux(tl.cons, tl.prim, tl.aux)
        flux_p = (flux + alpha * tl.cons) / 2
        flux_m = (flux - alpha * tl.cons) / 2
        flux_p_r = numpy.zeros_like(flux_p)
        flux_m_l = numpy.zeros_like(flux_m)
        Nvars, Npoints = (tl.Nvars, tl.grid.Npoints)
        ...
        rhs[:,1:-1] = 1/tl.grid.dx * (flux[:,1:-1] - flux[:,2:])
        if slow_source:
            rhs += slow_source(tl.cons, tl.prim, tl.aux)
        return rhs
    return fvs_solver
